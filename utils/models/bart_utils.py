...
def get_bart_layer_representations(args, text_array, remove_chars, word_ind_to_extract):
    seq_len = args.sequence_length
    nlp_model = args.nlp_model
    
    if nlp_model.startswith('bart-checkpoint'):
        assert ('bart' in args.checkpoint_dir)  # Small check that the right model is loaded.
        
        model = AutoModelForSeq2SeqLM.from_pretrained(args.checkpoint_dir).to(device)
        tokenizer = AutoTokenizer.from_pretrained('facebook/bart-base')
    else:
        if nlp_model == 'bart-base':
            hf_name = 'facebook/bart-base'
        elif nlp_model == 'bart-booksum':
    ...
    for word in text_array:
        current_token_embedding = get_bart_token_embeddings([word], tokenizer, model, remove_chars)
        token_embeddings.append(np.mean(current_token_embedding.detach().numpy(), 1))
    
    # where to store layer-wise BART embeddings of particular length
    BART = {}
    for layer in range(n_total_layers):
        BART[layer] = []
    BART[-1] = token_embeddings
    ...
    for truncated_seq_len in range(1, 1+seq_len):
        ...
        BART = add_avrg_token_embedding_for_specific_word(word_seq, tokenizer, model, remove_chars, 
                                                            from_start_word_ind_to_extract, BART)
...
@torch.inference_mode()
def predict_bart_embeddings(words_in_array, tokenizer, model, remove_chars):    
...
@torch.inference_mode()
def add_word_bart_embedding(model_dict, embeddings_to_add, token_inds_to_avrg, specific_layer=-1):
...
def add_avrg_token_embedding_for_specific_word(word_seq,tokenizer,model,remove_chars,from_start_word_ind_to_extract,model_dict):
    ...
    all_sequence_embeddings, word_ind_to_token_ind, _ = predict_bart_embeddings(word_seq, tokenizer, model, remove_chars)
    token_inds_to_avrg = word_ind_to_token_ind[from_start_word_ind_to_extract]
    model_dict = add_word_bart_embedding(model_dict, all_sequence_embeddings,token_inds_to_avrg)
    
    return model_dict


# get the BART token embeddings
@torch.inference_mode()
def get_bart_token_embeddings(words_in_array, tokenizer, model, remove_chars):    
    for word in words_in_array:
        if word in remove_chars:
            print('An input word is also in remove_chars. This word will be removed and may lead to misalignment. Proceed with caution.')
            return -1
