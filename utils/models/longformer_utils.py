...
def get_longformer_layer_representations(args, text_array, remove_chars, word_ind_to_extract):
    ...
    for word in text_array:
        current_token_embedding = get_longformer_token_embeddings([word], tokenizer, model, remove_chars)
        token_embeddings.append(np.mean(current_token_embedding.detach().numpy(), 1))
    
    # where to store layer-wise longformer embeddings of particular length
    Longformer = {}
    for layer in range(n_total_layers):
        Longformer[layer] = []
    Longformer[-1] = token_embeddings
    ...
    for truncated_seq_len in range(1, 1+seq_len):
        ...
        Longformer = add_avrg_token_embedding_for_specific_word(word_seq, tokenizer, model, remove_chars, 
                                                            from_start_word_ind_to_extract, Longformer)
...
@torch.inference_mode()
def predict_longformer_embeddings(words_in_array, tokenizer, model, remove_chars):    
    ...
    outputs = model(tokens_tensor, attention_mask=attention_mask, global_attention_mask=global_attention_mask,
                    output_hidden_states=True)
    encoded_layers = outputs['hidden_states'][1:]   # Start from 1, ignore layer 0 (initial embedding outputs)
    pooled_output = np.squeeze(model.pooler(encoded_layers[-1]).cpu().detach().numpy())
...
@torch.inference_mode()
def add_word_longformer_embedding(model_dict, embeddings_to_add, token_inds_to_avrg, specific_layer=-1):
...
def add_avrg_token_embedding_for_specific_word(word_seq,tokenizer,model,remove_chars,from_start_word_ind_to_extract,model_dict):
    ...
    all_sequence_embeddings, word_ind_to_token_ind, _ = predict_longformer_embeddings(word_seq, tokenizer, model, remove_chars)
    token_inds_to_avrg = word_ind_to_token_ind[from_start_word_ind_to_extract]
    model_dict = add_word_longformer_embedding(model_dict, all_sequence_embeddings,token_inds_to_avrg)
    
    return model_dict


# get the Longformer token embeddings
@torch.inference_mode()
def get_longformer_token_embeddings(words_in_array, tokenizer, model, remove_chars):    
    for word in words_in_array:
        if word in remove_chars:
            print('An input word is also in remove_chars. This word will be removed and may lead to misalignment. Proceed with caution.')
            return -1
